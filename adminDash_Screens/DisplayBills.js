...
const DisplayBills = ({navigation,route}) => {
  ...
  const DeleteBills = (userId) => {
    db.transaction((tx) => {
      // Execute the SQL query to delete the user
      tx.executeSql('DELETE FROM bill_table WHERE billId = ?', [userId], (txObj, resultSet) => {
        // Handle success (optional)
        Alert.alert('Bill deleted successfully.');
        navigation.replace('View Users Bill')
      },
      (txObj, error) => {
      ...
      return (
        <View
          ...
          Alert.alert('Warning!', 'Are you sure that you want to delete this', [
            ...
            {text: 'YES', onPress: () => DeleteBills(item.billId)},
          ]);
