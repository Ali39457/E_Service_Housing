...
const RemoveAdmin = ({navigation,route}) => {
  ...
  useEffect(() => {
    db.transaction((tx) => {
      tx.executeSql(
        'SELECT * FROM table_user WHERE roleType="Admin"',
        [],
        (tx, results) => {
          var temp = [];
  ...
  const DeleteUser = (userId) => {
    db.transaction((tx) => {
      ...
      tx.executeSql('DELETE FROM table_user WHERE user_id = ? AND roleType="Admin"', [userId], (txObj, resultSet) => {
        // Handle success (optional)
        Alert.alert('Admin deleted successfully.');
        navigation.replace('Remove Admin')
      },
      (txObj, error) => {
